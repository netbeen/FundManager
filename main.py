...
for (key, value) in purchase_info_dict_list_dict.items():
    ...
    for fundInfoDict in fundInfoDictList:
        if nextPurchaseInfoDictIndex == len(thisFundPurchaseInfoDictList):
            ...
            currentShare += thisFundPurchaseInfoDictList[nextPurchaseInfoDictIndex]['totalPrice'] / fundInfoDict[
                'value']
            nextPurchaseInfoDictIndex += 1
        fundInfoDict['unitPrice'] = currentCost / currentShare  # 计算每份持仓成本

    for fundInfoDict in fundInfoDictList:
        fundInfoDict['profitRate'] = (fundInfoDict['value'] - fundInfoDict['unitPrice']) / fundInfoDict[
            'unitPrice']  # 计算收益率
    ...
    print('利润率：', '%.4f' % (fundInfoDictList[-1]['profitRate'] * 100), '%')
    print('年化收益率: ', '%.2f' % (fundInfoDictList[-1]['profitRatePerYear'] * 100), '%')
